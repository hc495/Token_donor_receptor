    ...
    **kwargs,
    ...
    for sentence in sentences:
        ...
        if replaced_word is not None:
            ...
            word_begin, word_end = find_token_location_in_sentence(tokenizer, sentence, replaced_word)
            if word_begin is None or word_end is None:
                print(f"Warning: The replaced word '{replaced_word}' is not found in the sentence '{sentence}' after replacement. Skipping this sentence.")
                continue
        inputs = tokenizer(sentence, return_tensors="pt").to(model.device)
        inputs.input_ids = inputs.input_ids[:, :word_end] 
        inputs.attention_mask = inputs.attention_mask[:, :word_end]
    ...
    return {
        ...
        "hidden_states": res if save_hidden_states else [],
    }


def donator_exp_for_one_word(
    model,
    tokenizer,
    word,
    sentences,
    replaced_word = None,
    aggression = "max", # "mean" or "max"
    save_hidden_states = False,
    **kwargs,
):
    hs_target_tokens = []
    hs_post_target_tokens = []
    for sentence in sentences:
        word_begin, word_end = find_token_location_in_sentence(tokenizer, sentence, word)
        if word_begin is None or word_end is None:
            print(f"Warning: The word '{word}' is not found in the sentence '{sentence}'. Skipping this sentence.")
        ...
        outputs = model(**inputs, output_hidden_states=True)
        hidden_states = outputs.hidden_states
        hs_target_token_in_layer = []
        hs_post_target_token_in_layer = []
        for layer in range(len(hidden_states)):
            hs_target_token_in_layer.append(hidden_states[layer][0, word_begin:word_end, :].detach().to(torch.float32).cpu().numpy())
            hs_post_target_token_in_layer.append(hidden_states[layer][0, word_end:, :].detach().to(torch.float32).cpu().numpy())
        hs_target_tokens.append(hs_target_token_in_layer)
        hs_post_target_tokens.append(hs_post_target_token_in_layer)

    if not hs_target_tokens:
        return {
            "word": word,
            "similarities": [],
            "hidden_states": [],
    ...
    pre_processed_res = [] # Token Averaged
    for i in range(len(hs_target_tokens)):
        pre_processed_res.append([])
        for layer in range(len(hs_target_tokens[i])):
            pre_processed_res[i].append(np.mean(hs_target_tokens[i][layer], axis=0, keepdims=False))
    
    sim_res = []
    for layer in range(len(pre_processed_res[0])):
        sim_in_layer = []
        for i in range(len(pre_processed_res)):
            target_token_feature = pre_processed_res[i][layer]
            post_target_token_feature = hs_post_target_tokens[i][layer]
            temp_sim_res = np.dot(post_target_token_feature, target_token_feature) / (np.linalg.norm(post_target_token_feature, axis=1) * np.linalg.norm(target_token_feature) + 1e-8)
            if aggression == "mean":
                sim_in_layer.append(np.mean(temp_sim_res).item())
            elif aggression == "max":
                sim_in_layer.append(np.max(temp_sim_res).item())
        sim_res.append(np.mean(sim_in_layer).item())

    return {
        "word": word,
        "similarities": sim_res,
        "hidden_states": hs_target_tokens if save_hidden_states else [],
    }
...
def process_standard_dataset(
    ...
    experiment_type: str = "receptor",
    donator_aggression: str = "max",
    save_hidden_states: bool = False,
    replaced_source_key_word: str = None,
    replaced_container_key_word: str = None,
):
    if replaced_source_key_word is not None and replaced_container_key_word is not None:
        return __process_standard_dataset_with_replacement(
            model,
            tokenizer,
            dataset,
            replaced_source_key_word,
            replaced_container_key_word,
            experiment_type,
            donator_aggression,
            save_hidden_states
        )
    if experiment_type == "receptor":
        func = receptor_exp_for_one_word
    elif experiment_type == "donator":
        func = donator_exp_for_one_word
    else:
    ...
    for word_type, word_sentences_dict in tqdm(dataset.items(), desc="Processing dataset"):
        ...
        for word, sentences in tqdm(word_sentences_dict.items(), desc=f"Processing words in {word_type}", leave=False):
            res[word_type][word] = func(model, tokenizer, word, sentences, aggression=donator_aggression, save_hidden_states=save_hidden_states)
    return res


def __process_standard_dataset_with_replacement(
    model,
    tokenizer,
    dataset: dict[str, dict[str, list]],
